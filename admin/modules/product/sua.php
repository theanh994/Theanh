...
<!--                            <div class="input-item form-group">-->
<!--                                <label for="product_capacity" class="d-block">Size</label>-->
<!--                                <select name="product_capacity[]" id="product_capacity" class="form-control select_capacity" multiple>-->
<!--                                    <option value="0">Chưa xác định</option>-->
<!--                                    --><?php
//                                    // Lấy danh sách dung tích từ cơ sở dữ liệu
//                                    $sql_capacity_list = "SELECT * FROM capacity ORDER BY capacity_id ASC";
//                                    $query_capacity_list = mysqli_query($mysqli, $sql_capacity_list);
//
//                                    // Giả sử bạn đã có biến chứa dung tích hiện tại đã được lưu, ví dụ:
//                                    // $current_capacity_id = "1,2"; // Chuỗi dung tích đã chọn
//                                    $current_capacity_ids = explode(',', $row['capacity_id']); // Chuyển đổi thành mảng
//
//                                    // Lặp qua danh sách dung tích và hiển thị
//                                    while ($row_capacity = mysqli_fetch_array($query_capacity_list)) {
//                                        // Kiểm tra xem dung tích có trong danh sách hiện tại không
//                                        $selected = in_array($row_capacity['capacity_id'], $current_capacity_ids) ? 'selected' : '';
//                                        ?>
<!--                                        <option value="--><?php //echo $row_capacity['capacity_id']; ?><!--" --><?php //echo $selected; ?><!-->-->
<!--                                            --><?php //echo $row_capacity['capacity_name']; ?>
<!--                                        </option>-->
<!--                                        --><?php
//                                    }
//                                    ?>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-content">
                         ...
                         <div class="input-item form-group">
                            <label class="d-block" for="product_images">Images Multiple</label>
                            <div class="image-box w-100">
                                <figure class="image-container p-relative">
                                    <img id="chosen-image-multiple">
                                    <figcaption id="file-names"></figcaption>
                                </figure>
                                <input type="file" class="d-none" id="product_images" name="product_images[]" accept="image/*" multiple>
                                <label class="label-for-image" for="product_images">
                                    <i class="fas fa-upload"></i> &nbsp; Chọn nhiều hình ảnh
                                </label>
                            </div>
                        </div>

                        <!-- Image preview for existing images -->
                        <div id="image-preview-multiple" class="d-flex flex-wrap">
                            <?php foreach ($product_images as $image) { ?>
                                <img src="modules/product/uploads/<?php echo $image; ?>" alt="product image">
                            <?php } ?>
                        </div>
                        ...
                        <!-- Script to preview selected images -->
                        <script>
                            let multipleUploadButton = document.getElementById("product_images");
                            let imagePreviewMultiple = document.getElementById("image-preview-multiple");
                            let fileNames = document.getElementById("file-names");

                            multipleUploadButton.onchange = () => {
                                imagePreviewMultiple.innerHTML = ''; // Clear previous images
                                let fileArray = Array.from(multipleUploadButton.files);
                                ...
                                fileArray.forEach(file => {
                                    ...
                                    reader.onload = () => {
                                        ...
                                        imagePreviewMultiple.appendChild(img);
                                    }
                                });

                                // Display file names
                                fileNames.textContent = fileArray.map(file => file.name).join(', ');
                            };
                        </script>
                            <div class="input-item form-group">
