...
<div class="col-lg-12 grid-margin stretch-card">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">Doanh thu chi nhánh</h4>
            <div id="agencySalesChart"></div>
        </div>
    </div>
</div>
...
<?php
$query_agency_sales = mysqli_query($mysqli, "
    SELECT IFNULL(agency_name, 'Ngẫu nhiên chi nhánh') as agency_name, SUM(total_amount) as total_sales 
    FROM orders 
    WHERE order_date BETWEEN '$subdays' AND '$now'
    GROUP BY agency_name
");

$data_agency_sales = [];
while ($row = mysqli_fetch_assoc($query_agency_sales)) {
    $data_agency_sales[] = [
        'label' => $row['agency_name'],
        'value' => $row['total_sales']
    ];
}
$data_agency_sales_json = json_encode($data_agency_sales);
...
<script>
    $(document).ready(function() {
        var agencySalesData = <?php echo $data_agency_sales_json; ?>;

        new Morris.Bar({
            element: 'agencySalesChart',
            data: agencySalesData,
            xkey: 'label',
            ykeys: ['value'],
            labels: ['Doanh thu'],
            barColors: ['#0b62a4'],
            hideHover: 'auto',
            resize: true,
            xLabelMargin: 10, // Điều chỉnh khoảng cách nhãn trục X
            xLabelAngle: 0, // Xoay nhãn trục X
        });

        // Thay đổi kích thước font chữ
        $('#agencySalesChart text').css('font-size', '10px');
    });
