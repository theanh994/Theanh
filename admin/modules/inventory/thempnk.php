...
if (isset($_POST['inventory_add'])) {
    ...
    $agency_name = $_POST['agency_name']; // Lấy giá trị agency_name từ form
    $inventory_date = Carbon::now('Asia/Ho_Chi_Minh')->toDateString();

    // Thực hiện chèn dữ liệu vào bảng inventory, bao gồm cả agency_name
    $sql_insert_inventory = "INSERT INTO inventory(account_id, staf_name, supplier_name, supplier_phone, inventory_note, inventory_code, inventory_date, total_amount, inventory_status, agency_name) 
    VALUES ('".$_SESSION['account_id']."', '$staf_name', '$supplier_name', '$inventory_phone', '$inventory_note', '$inventory_code', '$inventory_date', '0', '0', '$agency_name')";
    ...
    if ($query_insert_inventory) {
        ...
        foreach ($_SESSION['inventory'] as $inventory_item) {
            ...
            VALUES ('" . $inventory_code . "', '" . $product_id . "', '" . $product_quantity . "', '" . $product_price_import . "')";
